...
<block data-type="template" data-is="chat-page-status" data-attr="chatStatue,chatStatusContent">
    <view :style="'background-color: ' + (chatStatue==='open'?'#40c9ff':'#888888') + ';'" class="chat-page-status-style">
        {{chatStatusContent}}
    </view>
</block>

<scroll-view enable-flex="true" :style="'height:' + pageHeight + 'px;width: 100%;display: flex;flex-direction: column;margin-top: 54rpx;'" scroll-y="true" :scroll-top="scrollTopVal" @tap="resetInputStatus">
    <block v-for="(item, index) in chatItems" :key="index">
        <block data-type="template" data-is="chat-item" data-attr="length:chatItems.length,index:index,item:item">
...
export default {
  ...
  methods: {
    ...
    onMessageLongTap(e) {
      ...
      let menuItems = ['删除', '复制']; // TODO 还需时间判断是否可以撤回
    ...
    onExtraItemClickEvent(e) {
      ...
      wx.chooseImage({
        ...
        success: res => {
          ...
          this.sendMessage(imageMessageContent); // this.msgManager.sendMsg({ type: IMOperator.ImageType, content: res.tempFilePaths[0] })
        }
      });
    },

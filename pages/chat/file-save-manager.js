    ...
    return new Promise((resolve, reject) => {
      wx.getFileInfo({
        ...
        success: tempFailInfo => {
          let tempFileSize = tempFailInfo.size; // console.log('本地临时文件大小', tempFileSize);
          ...
          wx.getSavedFileList({
            success: savedFileInfo => {
              ...
              if (!fileList) {
                reject({
                  errMsg: '获取到的fileList为空，请检查你的wx.getSavedFileList()函数的success返回值'
                });
                return;
              } //这里计算需要移除的总文件大小
                  ...
                  if ((sizeCount += fileList[i].size) >= sizeNeedRemove) {
                    for (let j = 0; j < i; j++) {
                      ...
                      wx.removeSavedFile({
                        ...
                        success: function () {
                          wholeSize -= fileList[j].size; // console.log('移除成功', wholeSize);
                        }
                      });
                    }

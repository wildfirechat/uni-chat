...
class WfcImpl {
    ...
    async connect(userId, token) {
        self.userId = userId;
        uniWfcClient.connect(Config.IM_SERVER_HOST, userId, token);
        console.log('connect', Config.IM_SERVER_HOST);
    ...
    addGroupMembers(groupId, memberIds, extra, notifyLines, notifyMessageContent, successCB, failCB) {
        ...
        let notifyContentStr = JSON.stringify(payload);
        extra = !extra ? '' : extra;
        uniWfcClient.addMembers(memberIds, groupId, extra, notifyLines, notifyContentStr,
            () => {
                if (successCB) {
                    successCB();
                }
    ...
    isHiddenGroupMemberName(groupId) {
        return uniWfcClient.isHiddenGroupMemberName(groupId);
    }

    async setHiddenGroupMemberName(groupId, hide, successCB, failCB) {
        uniWfcClient.setHiddenGroupMemberName(groupId, hide, () => {
            successCB();
        }, (errorCode) => {
            failCB(errorCode);
        });
    ...
    markConversationAsUnread(conversation, syncToOtherClient) {
        let msgUid = uniWfcClient.setLastReceivedMessageUnRead(JSON.stringify(conversation), '0', '0');
    ...
    getConversationDelivery(conversation) {
        let deliveryStr = uniWfcClient.getMessageDelivery(JSON.stringify(conversation));
    ...
    searchMessage(conversation, keyword) {
        let result = uniWfcClient.searchMessage(JSON.stringify(conversation), keyword);
    ...
    searchMessageEx(conversation, keyword, desc, limit, offset) {
        let result = uniWfcClient.searchMessageEx(JSON.stringify(conversation), keyword, desc, limit, offset);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    searchMessageByTypes(conversation, keyword, contentTypes, desc, limit, offset) {
        ...
        let result = uniWfcClient.searchMessageByTypes(JSON.stringify(conversation), keyword, contentTypes, desc, limit, offset);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    searchMessageByTypesAndTimes(conversation, keyword, contentTypes, startTime, endTime, desc, limit, offset) {
        ...
        let result = uniWfcClient.searchMessageByTypesAndTimes(JSON.stringify(conversation), keyword, contentTypes, startTime.toString(), endTime.toString(), desc, limit, offset);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    insertMessage(conversation, messageContent, status, notify = false, serverTime = 0) {
        let protoMessageContent = messageContent.encode();
        let mStr = uniWfcClient.insertMessage(JSON.stringify(conversation), self.userId, JSON.stringify(protoMessageContent), status, notify, serverTime);
        return Message.fromProtoMessage(JSON.parse(mStr));
    }

...
import utsWfcClient from "./utsWfcClient";
...
class WfcImpl {
    ...
    onReceiveMessage(messages, hasMore) {
        ...
        objMsgs.forEach(m => {
            ...
            if (msg.messageContent instanceof MarkUnreadMessageContent && msg.from === self.userId) {
                ...
                utsWfcClient.setLastReceivedMessageUnRead(JSON.stringify(conversation), stringValue(markMsg.messageUid), stringValue(markMsg.timestamp));
            }
            // 兼容旧版
            if (msg) {
    ...
    init(args = []) {
        ...
        //utsWfcClient.startBugly('f06e6c67c5');
    ...
    setFavUser(userId, fav, successCB, failCB) {
        utsWfcClient.setFavUser(userId, fav, () => {
            if (successCB) {
                successCB();
            }
        }, (errorCode) => {
    ...
    loadRemoteMessages(conversation, contentTypes, beforeUid, count, successCB, failCB) {
        ...
        utsWfcClient.getRemoteMessages(JSON.stringify(conversation), beforeUid + '', count, (protoMsgsStr) => {
    ...
    async createGroup(groupId, groupType, name, portrait, groupExtra = '', memberIds = [], memberExtra = '', lines = [0], notifyContent, successCB, failCB) {
        ...
        utsWfcClient.createGroup(groupId, groupType, name, portrait, groupExtra, memberIds, memberExtra, lines, notifyContentStr,
            (groupId) => {
                if (successCB) {
                    successCB(groupId);
                }
    ...
    addGroupMembers(groupId, memberIds, extra, notifyLines, notifyMessageContent, successCB, failCB) {
        ...
        let notifyContentStr = JSON.stringify(payload);
        extra = !extra ? '' : extra;
        utsWfcClient.addMembers(groupId, memberIds, extra, notifyLines, notifyContentStr,
            () => {
                if (successCB) {
                    successCB();
                }
    ...
    kickoffGroupMembers(groupId, memberIds, notifyLines, notifyMsg, successCB, failCB) {
        ...
        utsWfcClient.kickoffMembers(groupId, memberIds, notifyLines, strCont,
            () => {
                if (successCB) {
                    successCB();
                }
    ...
    async modifyGroupInfo(groupId, type, newValue, lines = [0], notifyMessageContent, successCB, failCB) {
        let payload = notifyMessageContent ? notifyMessageContent.encode() : '';
        utsWfcClient.modifyGroupInfo(groupId, type, newValue, lines, JSON.stringify(payload),
            () => {
                if (successCB) {
                    successCB();
                }
    ...
    async setFavGroup(groupId, fav, successCB, failCB) {
        utsWfcClient.setFavGroup(groupId, fav, () => {
            if (successCB) {
                successCB();
            }
        }, (errorCode) => {
    ...
    isHiddenGroupMemberName(groupId) {
        const setting = utsWfcClient.getUserSetting(UserSettingScope.GroupHideNickname, groupId);
        return '1' === setting;
    }
    ...
    getConversationInfo(conversation) {
        let convStr = utsWfcClient.getConversationInfo(JSON.stringify(conversation));
    ...
    async removeConversation(conversation, clearMsg = false) {
        utsWfcClient.removeConversation(JSON.stringify(conversation), clearMsg);
    }

    setConversationTop(conversation, top, successCB, failCB) {
        utsWfcClient.setConversationTop(JSON.stringify(conversation), top, () => {
    ...
    setConversationSlient(conversation, silent, successCB, failCB) {
        utsWfcClient.setConversationSlient(JSON.stringify(conversation), silent, () => {
            let conversationInfo = self.getConversationInfo(conversation);
            self.eventEmitter.emit(EventType.ConversationInfoUpdate, conversationInfo);
    ...
    setConversationDraft(conversation, draft = '') {
        ...
        utsWfcClient.setConversationDraft(JSON.stringify(conversation), draft);
    ...
    setConversationTimestamp(conversation, timestamp) {
        timestamp = timestamp + '';
        utsWfcClient.setConversationTimestamp(JSON.stringify(conversation), timestamp);
        let conversationInfo = self.getConversationInfo(conversation);
        self.eventEmitter.emit(EventType.ConversationInfoUpdate, conversationInfo);
    }
    ...
    getConversationUnreadCount(conversation) {
        let unreadCountStr = utsWfcClient.getConversationUnreadCount(JSON.stringify(conversation));
        return Object.assign(new UnreadCount(), JSON.parse(unreadCountStr));
    }

    clearConversationUnreadStatus(conversation) {
        console.log('clearConversationUnreadStatus', JSON.stringify(conversation));
        utsWfcClient.clearUnreadStatus(JSON.stringify(conversation));
        let conversationInfo = self.getConversationInfo(conversation);
        self.eventEmitter.emit(EventType.ConversationInfoUpdate, conversationInfo);
    }

    markConversationAsUnread(conversation, syncToOtherClient) {
        let result = utsWfcClient.setLastReceivedMessageUnRead(JSON.stringify(conversation), '0', '0');
        if (result) {
            let conversationInfo = self.getConversationInfo(conversation);
            self.eventEmitter.emit(EventType.ConversationInfoUpdate, conversationInfo);
        }
    ...
    getConversationRead(conversation) {
        let readedStr = utsWfcClient.getConversationRead(JSON.stringify(conversation));
    ...
    getConversationFirstUnreadMessageId(conversation) {
        let messageId = utsWfcClient.getConversationFirstUnreadMessageId(JSON.stringify(conversation));
        return messageId;
    }
    ...
    getMessages(conversation, fromIndex, before = true, count = 20, withUser = '', contentTypes = []) {
        let protoMsgsStr = utsWfcClient.getMessages(JSON.stringify(conversation), contentTypes, fromIndex, before, count, withUser);
    ...
    getUserMessages(userId, conversation, fromIndex, before = true, count = 20, contentTypes = []) {
        let protoMsgsStr = utsWfcClient.getUserMessages(userId, JSON.stringify(conversation), contentTypes, fromIndex, before, count);
        var protoMsgs = JSON.parse(protoMsgsStr);
        let msgs = [];
        protoMsgs.map(m => {
            let msg = Message.fromProtoMessage(m);
    ...
    getMessagesV2(conversation, fromIndex, before, count, withUser, successCB, failCB) {
        utsWfcClient.getMessagesV2(JSON.stringify(conversation), fromIndex, before, count, withUser, ...this._wrapperGetMessageCallbacks(successCB, failCB));
    }
    ...
    getMessagesByTimestampV2(conversation, contentTypes, timestamp, before, count, withUser, successCB, failCB) {
        utsWfcClient.getMessagesByTimestampV2(JSON.stringify(conversation), contentTypes, numberValue(timestamp), before, count, withUser, ...this._wrapperGetMessageCallbacks(successCB, failCB));
    }

    getUserMessagesV2(userId, conversation, fromIndex, before, count, successCB, failCB) {
        utsWfcClient.getUserMessagesV2(userId, JSON.stringify(conversation), fromIndex, before, count, ...this._wrapperGetMessageCallbacks(successCB, failCB));
    }
    ...
    searchMessage(conversation, keyword, withUser = '') {
        let result = utsWfcClient.searchMessage(JSON.stringify(conversation), keyword, withUser);
    ...
    searchMessageEx(conversation, keyword, desc, limit, offset, withUser) {
        let result = utsWfcClient.searchMessageEx(JSON.stringify(conversation), keyword, desc, limit, offset, withUser);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    searchMessageByTypes(conversation, keyword, contentTypes, desc, limit, offset, withUser = '') {
        ...
        let result = utsWfcClient.searchMessageByTypes(JSON.stringify(conversation), keyword, contentTypes, desc, limit, offset, withUser);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    searchMessageByTypesAndTimes(conversation, keyword, contentTypes, startTime, endTime, desc, limit, offset, withUser) {
        ...
        let result = utsWfcClient.searchMessageByTypesAndTimes(JSON.stringify(conversation), keyword, contentTypes, startTime.toString(), endTime.toString(), desc, limit, offset, withUser);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    async sendMessageEx(message, toUsers = [], preparedCB, progressCB, successCB, failCB) {
        ...
        utsWfcClient.sendMessage(strConv, strCont, toUsers, 0,
            (args) => { //preparedCB
    ...
    async recallMessage(messageUid, successCB, failCB) {
        ...
        utsWfcClient.recall(messageUid.toString(),
            () => {
    ...
    async deleteRemoteMessage(messageUid, successCB, failCB) {
        ...
        utsWfcClient.deleteRemoteMessage(messageUid.toString(),
            () => {
                if (successCB) {
                    successCB();
                }
    ...
    async updateRemoteMessageContent(messageUid, messageContent, distribute, updateLocal, successCB, failCB) {
        ...
        utsWfcClient.updateRemoteMessageContent(messageUid.toString(), JSON.stringify(protoMessageContent), distribute, updateLocal,
            () => {
                if (successCB) {
                    successCB();
                }
    ...
    watchOnlineState(conversationType, targets, duration, successCB, failCB) {
        utsWfcClient.watchOnlineState(conversationType, targets, duration,
            (strStates) => {
    ...
    unwatchOnlineState(conversationType, targets, successCB, failCB) {
        utsWfcClient.unwatchOnlineState(conversationType, targets,
            () => {
                if (successCB) {
                    successCB();
                }
    ...
    getConversationFileRecords(conversation, fromUser, beforeUid, count, successCB, failCB) {
        utsWfcClient.getConversationFiles(JSON.stringify(conversation), fromUser, Long.fromValue(beforeUid).toString(), count, (frsStr) => {
    ...
    async clearMessages(conversation) {
        utsWfcClient.clearMessages(JSON.stringify(conversation));
    ...
    async clearRemoteConversationMessages(conversation, successCB, failCB) {
        utsWfcClient.clearRemoteConversationMessages(JSON.stringify(conversation),
            () => {
                successCB && successCB();
            },
            (errorCode) => {
    ...
    async clearMessagesByTime(conversation, before) {
        utsWfcClient.clearMessagesByTime(JSON.stringify(conversation), before);
        let conversationInfo = this.getConversationInfo(conversation);
        self.eventEmitter.emit(EventType.ConversationInfoUpdate, conversationInfo);
    }
    ...
    insertMessage(conversation, messageContent, status, notify = false, toUsers = [], serverTime = 0) {
        let protoMessageContent = messageContent.encode();
        utsWfcClient.insertMessage(JSON.stringify(conversation), self.userId, JSON.stringify(protoMessageContent), status, notify, toUsers, serverTime);
    }
    ...
    async uploadMedia(fileName, data, mediaType, successCB, failCB, progressCB) {
        ...
        utsWfcClient.uploadMedia(fileName, data, mediaType,
            (remoteUrl) => {
                if (successCB) {
                    successCB(remoteUrl);
                }
    ...
    sendConferenceRequest(sessionId, roomId, request, data, advance, callback) {
        utsWfcClient.sendConferenceRequest('' + sessionId, roomId, request, data,
            (resp) => {
    ...
    searchFiles(keyword, conversation, fromUser, beforeMessageId, count, successCB, failCB) {
        utsWfcClient.searchFiles(keyword, conversation ? JSON.stringify(conversation) : '', fromUser, Long.fromValue(beforeMessageId).toString(), count,
            (files) => {
    ...
    searchMyFiles(keyword, beforeMessageUid, count, successCB, failCB) {
        utsWfcClient.searchMyFiles(keyword, Long.fromValue(beforeMessageUid).toString(), count,
            (files) => {
                let frs = JSON.parse(files);
